...
1. **다른 pfd 브랜치 참조 금지**: `{BRANCH_NAME}` 이외의 pfd 브랜치를 checkout, diff, log, read하지 마세요.
2. **미완성 시 타 브랜치 대체 금지**: 개발 스킬이 완료되지 않으면 다른 브랜치 코드로 채우지 말고 실패로 보고하세요.
3. **허용된 참조 소스**: `{BASE_BRANCH}` 기준 브랜치 코드, `{PLAN_FILE}` PLAN 파일, 개발 스킬이 생성한 코드만 사용하세요.
