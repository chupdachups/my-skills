...
[브랜치 독립성 규칙 - 반드시 준수]
- 현재 브랜치(${BRANCH_NAME}) 외의 다른 pfd 브랜치를 참조하지 마세요.
- 다른 worktree 디렉토리의 파일을 읽거나 복사하지 마세요.
- 개발이 완료되지 않으면 다른 브랜치 코드로 채우지 말고 그대로 종료하세요.
- 참조 허용 소스: 기준 브랜치 코드, PLAN 파일, 직접 생성한 코드만 사용하세요.
